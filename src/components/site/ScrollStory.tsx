...
import { ArrowRight, ArrowUpRight, Bot, Braces, Box, Globe2, PenTool, Workflow } from "lucide-react";
...
function StoryCard({ index, serviceIndex, projectIndex, progress, light }: StoryCardProps) {
  ...
  const Icon = cardIcons[serviceIndex] ?? Globe2;
  ...
  return <motion.article className={`story-card story-card-${index + 1}`} style={{ x, y, rotateY, rotateZ, scale }}>
    ...
    <div className="relative z-10 flex h-full flex-col justify-between">
      <div className="flex items-center justify-between text-[10px] uppercase">
        <span className="story-pill">{light ? project.type : service.id}</span>
        <span>MW / 0{light ? projectIndex + 1 : serviceIndex + 1}</span>
      </div>
      <div>
        <motion.div animate={{ opacity: light ? 0 : 1, y: light ? -12 : 0 }} className={light ? "pointer-events-none absolute" : ""}>
          <Icon className="mb-5 h-6 w-6" />
          <h3 className="text-2xl font-semibold md:text-3xl">{service.title}</h3>
          <p className="mt-2 max-w-52 text-xs opacity-70">{service.blurb}</p>
        </motion.div>
        <motion.div animate={{ opacity: light ? 1 : 0, y: light ? 0 : 12 }} className={!light ? "pointer-events-none absolute" : ""}>
          <p className="mb-4 text-[10px] uppercase opacity-65">Featured release</p>
          <h3 className="text-2xl font-semibold md:text-3xl">{project.name}</h3>
          <p className="mt-2 text-xs opacity-70">{project.tech}</p>
        </motion.div>
      </div>
    </div>
  </motion.article>;
...
export function ScrollStory() {
  ...
  useMotionValueEvent(scrollYProgress, "change", (value) => {
    const shouldBeLight = value >= 0.59;
    setLight((current) => current === shouldBeLight ? current : shouldBeLight);
  });

  if (reducedMotion) return <section className="story-static px-5 py-24 md:px-10"><div className="mx-auto max-w-6xl"><p className="text-xs uppercase text-muted-foreground">Built across every layer</p><h2 className="text-big mt-5 max-w-3xl">We build what moves businesses forward.</h2><div className="mt-12 grid gap-4 md:grid-cols-3">{services.slice(0, 6).map((service) => <div key={service.id} className="story-static-card p-6"><p className="text-xs text-primary">{service.id}</p><h3 className="mt-12 text-2xl font-semibold">{service.title}</h3><p className="mt-3 text-sm text-muted-foreground">{service.blurb}</p></div>)}</div></div></section>;

  const slots = [0, 1, 2] as const;
  return <section ref={sectionRef} className="story-scroll relative h-[520vh]">
    <div className="story-stage sticky top-0 h-[100svh] overflow-hidden">
      <div className="story-light absolute inset-0" />
      <motion.div className="story-dark absolute inset-0" style={{ opacity: darkOpacity }} />
      <motion.div className="absolute inset-0 z-10 flex items-center justify-center text-center" style={{ opacity: introOpacity }}><div><p className="text-xs uppercase text-story-muted">MakeWebb studio system</p><h2 className="mt-4 text-4xl font-semibold text-story-strong md:text-7xl">One team. Every layer.</h2></div></motion.div>
      ...
      <motion.div className="story-card-field absolute inset-0 z-20 flex items-center justify-center" style={{ opacity: cardsOpacity }}>
        {slots.map((index) => <StoryCard key={`service-${index}`} index={index} serviceIndex={index} projectIndex={index} progress={scrollYProgress} light={light} />)}
      </motion.div>
