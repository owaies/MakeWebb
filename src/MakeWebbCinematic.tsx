...
const slot = { x: [-1.72, -0.86, 0, 0.86, 1.72], y: [30, 4, -10, 4, 30], z: [0, 185, 470, 185, 0], ry: [25, 10, 0, -10, -25], scale: [0.63, 0.85, 1.04, 0.85, 0.63], opacity: [0.28, 0.78, 1, 0.78, 0.28] };
function clamp(v: number, a = 0, b = 1) { return Math.max(a, Math.min(b, v)); }
function cyc(values: number[], t: number) { const n = values.length, x = ((t % n) + n) % n, a = Math.floor(x), b = (a + 1) % n, f = x - a; return values[a] + (values[b] - values[a]) * f; }
...
function PersistentHeroCard({ progress }: { progress: ReturnType<typeof useSpring> }) {
  const enter = (p: number) => clamp((p - 0.17) / 0.10);
  const orbit = (p: number) => 2 + ((p - 0.27) / 0.51) * 5;
  const reverse = (p: number) => clamp((p - 0.78) / 0.12);
  const x = useTransform(progress, p => { if (p < 0.17) return '31vw'; if (p < 0.27) return `${31 + (0 - 31) * enter(p)}vw`; if (p < 0.78) return `${cyc(slot.x, orbit(p)) * 24.5}vw`; return `${cyc(slot.x, orbit(0.78)) * 24.5 * (1 - reverse(p)) + 31 * reverse(p)}vw`; });
  const y = useTransform(progress, p => { if (p < 0.17) return -10; if (p < 0.27) return -10 + 0 * enter(p); if (p < 0.78) return cyc(slot.y, orbit(p)); return cyc(slot.y, orbit(0.78)) * (1 - reverse(p)) - 10 * reverse(p); });
  const z = useTransform(progress, p => { if (p < 0.17) return 120; if (p < 0.27) return 120 + 350 * enter(p); if (p < 0.78) return cyc(slot.z, orbit(p)); return cyc(slot.z, orbit(0.78)) * (1 - reverse(p)) + 120 * reverse(p); });
  const ry = useTransform(progress, p => { if (p < 0.17) return -8; if (p < 0.27) return -8 + 8 * enter(p); if (p < 0.78) return cyc(slot.ry, orbit(p)); return cyc(slot.ry, orbit(0.78)) * (1 - reverse(p)) - 8 * reverse(p); });
  const scale = useTransform(progress, p => { if (p < 0.17) return 1.12; if (p < 0.27) return 1.12 - 0.08 * enter(p); if (p < 0.78) return cyc(slot.scale, orbit(p)); return cyc(slot.scale, orbit(0.78)) * (1 - reverse(p)) + 1.12 * reverse(p); });
  return <motion.a href="#contact" className="mw-card mw-persistent-card mw-accent-purple" style={{ x, y, z, rotateY: ry, scale }}><CardFace title={projects[0][0]} tag={projects[0][1]} tech={projects[0][2]} accent={projects[0][3]} index={0} /></motion.a>;
}

function OrbitCard({ index, progress }: { index: number; progress: ReturnType<typeof useSpring> }) {
  const entrance = useTransform(progress, p => clamp((p - 0.23) / 0.12));
  const exit = useTransform(progress, p => 1 - clamp((p - 0.76) / 0.10));
  const t = useTransform(progress, p => 2 + ((p - 0.27) / 0.51) * 5 + index);
  const x = useTransform(t, tv => `${cyc(slot.x, Number(tv)) * 24.5}vw`);
  ...
  const opacity = useTransform([t, entrance, exit], ([tv, e, ex]) => cyc(slot.opacity, Number(tv)) * Number(e) * Number(ex));
  const p = projects[index] as Project;
  return <motion.a href="#contact" className={`mw-card mw-orbit-card mw-accent-${p[3]}`} style={{ x, y, z, rotateY: ry, scale, opacity }}><CardFace title={p[0]} tag={p[1]} tech={p[2]} accent={p[3]} index={index} /></motion.a>;
}

function CinematicScene() {
  const ref = useRef<HTMLElement | null>(null);
  const { scrollYProgress } = useScroll({ target: ref, offset: ['start start', 'end end'] });
  const progress = useSpring(scrollYProgress, { stiffness: 125, damping: 30, mass: 0.5 });
  const [state, setState] = useState('HERO');
  useEffect(() => progress.on('change', p => setState(p < 0.27 ? 'HERO' : p < 0.56 ? 'ORBIT' : p < 0.64 ? 'BEYOND' : 'ORBIT')), [progress]);
  const white = useTransform(progress, [0, 0.16, 0.24, 0.82, 0.90], [0, 0, 1, 1, 0]);
  const black = useTransform(progress, [0.56, 0.61, 0.64], [0, 1, 0]);
  const blackY = useTransform(progress, [0.56, 0.61, 0.64], ['100%', '0%', '-100%']);
  const type1 = useTransform(progress, [0.25, 0.38, 0.55], ['120vw', '0vw', '-125vw']);
  const type1o = useTransform(progress, [0.23, 0.27, 0.51, 0.57], [0, 1, 1, 0]);
  const type2 = useTransform(progress, [0.50, 0.62, 0.76], ['-120vw', '0vw', '120vw']);
  const type2o = useTransform(progress, [0.48, 0.53, 0.71, 0.78], [0, 1, 1, 0]);
  const heroOpacity = useTransform(progress, [0.15, 0.24, 0.84, 0.91], [1, 0, 0, 1]);
  const heroScale = useTransform(progress, [0, 0.24, 0.84, 0.91], [1, 0.94, 0.94, 1]);
  return <section ref={ref} id="work" className="mw-cinematic"><div className="mw-sticky">
    <motion.div className="mw-white-stage" style={{ opacity: white }} />
    <motion.div className="mw-hero-layer" style={{ opacity: heroOpacity, scale: heroScale }}><div className="mw-meta"><span>MAKEWEBB / 001 · DIGITAL PRODUCT STUDIO</span><span>AI · WEB · DATA · EXPERIENCE</span></div><div className="mw-hero-copy"><small>MAKEWEBB / 2026</small><h1>Systems that<br /><em>answer first.</em></h1><p>We design and build intelligent digital products across web, apps, AI, data and interactive experiences.</p></div><div className="mw-hero-bottom"><span>✦</span><span>SCROLL / EXPLORE</span></div></motion.div>
