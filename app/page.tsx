...
import { ArchitecturePipeline, CursorContextBridge, MakeWebbLogo, ProcessJourney, ProjectConfigurator, SkillConstellation } from './studio-features';
...
function useAnimeMotion() {
  useEffect(() => {
    ...
    const revealTargets=document.querySelectorAll('.section-head, .service-card, .person-card, .process-item, .contact-card, .interactive-block, .architecture, .configurator');
...
export default function Home(){
  ...
  return <main>
    ...
    <section className="section studio-section" id="studio"><div className="shell"><div className="section-head studio-head"><div><span className="section-number">07 / THE STUDIO</span><h2>Two minds.<br/><span>One build system.</span></h2></div><p>MakeWebb brings together AI/ML engineering and web development to move ideas from concept to shipped product.</p></div><div className="people-grid">{people.map(p=><TiltCard className="person-card" key={p.name}><div className="person-image-wrap"><img src={p.image} alt={p.name} className="person-image"/><div className="image-grid"/></div><div className="person-info"><span className="person-tag">{p.tag}</span><h3>{p.name}</h3><p>{p.role}</p><SkillConstellation person={p.name}/><div className="person-links"><a href={p.portfolio} target="_blank" rel="noreferrer" data-cursor="OPEN">Portfolio <ArrowUpRight size={14}/></a><a href={p.github} target="_blank" rel="noreferrer" data-cursor="OPEN">GitHub <ArrowUpRight size={14}/></a><a href={p.linkedin} target="_blank" rel="noreferrer" data-cursor="OPEN"><Linkedin size={14}/></a></div></div></TiltCard>)}</div></div></section>
    <section className="section shell" id="configurator"><div className="section-head"><div><span className="section-number">08 / START A PROJECT</span><h2>Configure<br/><span>the build.</span></h2></div><p>Choose the shape, budget and pace. Your selections turn into a ready-to-send project brief in one click.</p></div><ProjectConfigurator/></section>
    <section className="contact-section shell" id="contact"><div className="contact-card"><div className="contact-orb"><Code2 size={42}/><span/></div><div className="contact-copy"><span className="section-number">09 / CONTACT</span><h2>Have an idea?<br/><em>Let's make it real.</em></h2><p>Prefer a direct conversation? Choose who you want to reach at MakeWebb.</p></div><div className="contact-actions"><span className="contact-prompt">CHOOSE WHO TO CONTACT</span><a className="contact-person-button" href="mailto:owaies786@gmail.com" aria-label="Email Mohammed Owaies" data-cursor="OPEN"><span><strong>Mohammed Owaies</strong><small>AI / ML Engineer</small></span><Mail size={19}/></a><a className="contact-person-button" href="mailto:kingahassan786@gmail.com" aria-label="Email Mohammed Afaf Hassan" data-cursor="OPEN"><span><strong>Mohammed Afaf Hassan</strong><small>Web Developer</small></span><Mail size={19}/></a></div></div></section>
