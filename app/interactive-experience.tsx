...
export function TechStackOrbit() {
  ...
  return <div className="tech-orbit-wrap"><div className="tech-orbit-scene"><div className="tech-orbit-ring ring-one"/><div className="tech-orbit-ring ring-two"/><div className="tech-core"><Cpu size={22}/><strong>{active}</strong><small>BUILD SYSTEM</small></div>{technologies.map((tech, i) => <button key={tech} className="tech-node" style={{ '--angle': `${i * 45}deg`, '--radius': i % 2 === 0 ? 'clamp(100px, 28vw, 150px)' : 'clamp(112px, 32vw, 178px)' } as React.CSSProperties} onMouseEnter={() => setActive(tech)} onFocus={() => setActive(tech)} data-cursor="EXPLORE">{tech}</button>)}</div><div className="tech-orbit-copy"><span>STACK / ORBIT</span><h3>Tools orbit the idea.</h3><p>Hover a technology to bring it into focus. The stack moves, but the center stays the same: the product.</p><div><Wand2 size={15}/> RESPONSIVE TO POINTER</div></div></div>;
}
