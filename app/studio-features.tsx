...
export function MakeWebbLogo({ mode = 'lime', compact = false }: { mode?: 'lime' | 'black' | 'white'; compact?: boolean }) {
  ...
  return <div className={`makewebb-logo ${compact ? 'compact' : ''}`} style={{ color: textColor }} aria-label="MAKEWEBB">
...
export function LogoSystem() {
  return <div className="logo-system">
    <div className="logo-system-card dark"><span>PRIMARY</span><MakeWebbLogo mode="lime"/></div>
    <div className="logo-system-card light"><span>BLACK</span><MakeWebbLogo mode="black"/></div>
    <div className="logo-system-card dark"><span>WHITE</span><MakeWebbLogo mode="white"/></div>
    <div className="logo-system-card dark icon-card"><span>ICON / FAVICON</span><MakeWebbLogo mode="lime" compact/></div>
  </div>;
}
...
export function ProcessJourney() {
  ...
  useEffect(() => {
    const observers = refs.current.map((node, i) => {
      if (!node) return null;
      const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) setActive(i); }, { threshold: 0.6, rootMargin: '-20% 0px -20% 0px' });
      observer.observe(node); return observer;
    });
    return () => observers.forEach(o => o?.disconnect());
  }, []);
  const item = journey[active]; const Icon = item.icon;
  return <div className="journey-shell">
    ...
    <div className="journey-list">{journey.map((stage, i) => { const StageIcon = stage.icon; return <button ref={node => { refs.current[i] = node; }} className={`journey-stage ${i === active ? 'active' : ''}`} key={stage.num} onClick={() => setActive(i)} data-cursor="EXPLORE">
      <span className="journey-stage-num">{stage.num}</span><span className="journey-stage-icon"><StageIcon size={18}/></span><span className="journey-stage-copy"><strong>{stage.title}</strong><small>{stage.text}</small></span>{i < journey.length - 1 && <ArrowDown className="journey-down" size={15}/>}</button>; })}</div>
    <div className="journey-visual" aria-live="polite"><div className={`journey-object ${item.object}`}><span>{item.num}</span><i/><b/></div><div className="journey-visual-copy"><span>STAGE / {item.num}</span><h3>{item.title}</h3><p>{item.text}</p><div><Icon size={14}/> ACTIVE ROUTE</div></div></div>
  </div>;
}

export function ArchitecturePipeline() {
  const [active, setActive] = useState(0);
  return <div className="architecture"><div className="architecture-line" aria-hidden="true"><span style={{ width: `${(active / (pipeline.length - 1)) * 100}%` }}/></div><div className="architecture-nodes">
...
export function ProjectConfigurator() {
  const [product, setProduct] = useState('Website'); const [budget, setBudget] = useState(125000); const [timeline, setTimeline] = useState('1 MONTH');
  ...
  return <div className="configurator"><div className="config-head"><div><span>PROJECT CONFIGURATOR / 08</span><h3>What are you building?</h3></div><div className="config-live"><i/> CONFIG READY</div></div><div className="config-body">
  ...
  </div><div className="config-footer"><div><small>YOUR BUILD</small><strong>{summary}</strong></div><a className="button primary" href={`mailto:owaies786@gmail.com?subject=${encodeURIComponent(`MAKEWEBB project: ${product}`)}&body=${encodeURIComponent(`Hi MAKEWEBB,\n\nI want to build: ${product}\nBudget: ${formatted}\nTimeline: ${timeline}\n\nLet's discuss the project.`)}`} data-cursor="OPEN">BUILD MY PROJECT <ArrowRight size={16}/></a></div></div>;
}

export function CursorContextBridge() {
  useEffect(() => {
    const cursor = document.querySelector('.anime-cursor') as HTMLElement | null; if (!cursor) return;
    const label = cursor.querySelector('span') as HTMLElement | null; if (!label) return;
