...
export default function AnimeEnhancements() {
  useEffect(() => {
    ...
    scope.execute(() => {
      ...
      if (nav && !reduced) {
        animate(nav, { opacity: [0, 1], y: [-12, 0], duration: 620, ease: 'out(4)' });
      }
      ...
      if (brand && !reduced) {
        animate(brand, { rotateZ: [-10, 0], scale: [0.88, 1], duration: 760, delay: 120, ease: 'out(4)' });
      }
      ...
      hoverTargets.forEach((node) => {
        const enter = () => {
          if (coarse || reduced) return;
          animate(node, { translateY: -3, duration: 180, ease: 'out(3)' });
        };
        const leave = () => {
          if (coarse || reduced) return;
          animate(node, { translateY: 0, duration: 260, ease: 'out(3)' });
        };
      ...
      if (!reduced) {
        animate(sectionNumbers, { opacity: [0.55, 1], letterSpacing: ['0.1em', '0.16em'], duration: 700, delay: stagger(60), ease: 'out(3)' });
      }
    });
